...
namespace UserApp
{
    public partial class ChatbotDemo : Form
    {
        // ⚠️ LƯU Ý: Thay bằng API Key thật của bạn
        ...
        {
            InitializeComponent();
            btnSend.Click += btnSend_Click; // Gán sự kiện click
        }
        ...
        private string ProcessUserQuestion(string msg)
        {
            ...
            {
                return DatabaseHelper2.GetShowtimesByMovie(movieName);
            }
         ...
         msg.Contains("đặt vé") || msg.Contains("mua vé")) && !string.IsNullOrEmpty(movieName))
            {
                ...
                string data = DatabaseHelper2.GetShowtimesByMovie(movieName);
            ...
            if (!string.IsNullOrEmpty(genre))
            {
                ...
                return DatabaseHelper2.SuggestNowOrSoonByGenre(genre);
            }
        ...
        msg == "gợi ý phim" || msg == "gợi ý phim cho mình")
            {
                ...
                if (string.IsNullOrEmpty(genre))
                {
                    ...
                    result = DatabaseHelper2.GetMoviesInTheaters();
                }
                else
                {
                    // B3.2: Có thể loại -> Tìm phim theo thể loại
                    result = DatabaseHelper2.SuggestNowOrSoonByGenre(genre);
            ...
            {
                return DatabaseHelper2.GetSeatPricesSummary();
            }

            // 4. Hỏi phim đang chiếu
            if (msg.Contains("đang chiếu") || msg.Contains("phim mới"))
            {
                return DatabaseHelper2.GetMoviesInTheaters();
            }

            // 5. Hỏi phim sắp chiếu
            if (msg.Contains("sắp chiếu"))
            {
                return DatabaseHelper2.GetComingSoonMovies();
            }
